...
{
    Impl(WorkflowId workflow_id, std::shared_ptr<CancellationToken::Flag> flag)
...
WorkflowContext::WorkflowContext(WorkflowId workflow_id, std::shared_ptr<CancellationToken::Flag> flag)
    : m_impl(std::make_shared<Impl>(workflow_id, std::move(flag)))
{
}
...
{
    Impl(WorkflowId workflow_id, CancellationToken token)
        : workflow_id(workflow_id)
...
TaskContext::TaskContext(WorkflowId workflow_id, CancellationToken token)
    : m_impl(std::make_shared<Impl>(workflow_id, std::move(token)))
{
}
